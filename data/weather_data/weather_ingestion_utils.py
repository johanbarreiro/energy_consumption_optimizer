...
def calculate_heat_index(df):
    ...
    df['humidity_percentage'] = df['humidity'] * 100

    # Calculate the heat index only for temperatures above 80°F, otherwise return the temperature in Celsius
    df['heat_index_f'] = np.where(
        df['temp_f'] < 80,
        df['temp_f'],
        (
            c1 + (c2 * df['temp_f']) + (c3 * df['humidity_percentage']) +
            (c4 * df['temp_f'] * df['humidity_percentage']) +
            (c5 * (df['temp_f'] ** 2)) +
            (c6 * (df['humidity_percentage'] ** 2)) +
            (c7 * (df['temp_f'] ** 2) * df['humidity_percentage']) +
            (c8 * df['temp_f'] * (df['humidity_percentage'] ** 2)) +
            (c9 * (df['temp_f'] ** 2) * (df['humidity_percentage'] ** 2))
        )
    )
    ...
    df.drop(columns=['temp_f', 'humidity_percentage', 'heat_index_f'], inplace=True)

    return df


def process_weather_data_to_csv(input_filepath, output_filepath, amount_of_columns='essential'):
    weather_data = fetch_weather_data_from_csv(input_filepath,
                                                   amount_of_columns=amount_of_columns)
    weather_data = approximate_hourly_weather(weather_data)

    weather_data = calculate_heat_index(weather_data)

    weather_data.to_csv(output_filepath, index=False)

    print(f"Processed weather data saved to '{output_filepath}'.")